...
function signColor(v: number | undefined | null): string {
  ...
  return v > 0 ? "text-[#b6ff3c]" : "text-[#ff6a4d]";
}
...
function KindBadge({ kind }: { kind: string }) {
  ...
  return (
    <span
      className={`d8-chip font-semibold ${
        cross ? "bg-[#5a8cff]/15 text-[#5a8cff]" : "bg-[#b6ff3c]/12 text-[#b6ff3c]"
      }`}
    >
      {cross ? "cross" : "carry"}
    </span>
  ...
  return (
    <main className="mx-auto w-full max-w-6xl flex-1 px-5 py-10">
      <header className="mb-8 flex flex-wrap items-center gap-3">
        <span className="d8-pulse inline-block h-2.5 w-2.5 rounded-[2px] bg-[#b6ff3c]" />
        <div className="min-w-0 flex-1">
          <h1 className="d8-display text-2xl">
            DELTA/8 <span className="font-normal text-[var(--muted)]">— control room</span>
          </h1>
          <p className="text-[11px] tracking-[0.08em] text-[var(--muted)]">
            BINANCE · BYBIT · OKX — CROSS-EXCHANGE &amp; CASH-AND-CARRY
          </p>
        </div>
        <Link
          href="/bot"
          className="text-xs tracking-[0.04em] text-[var(--muted)] hover:text-[#b6ff3c]"
        >
          ← LANDING
        </Link>
        <Link
      ...
      {/* status bar */}
      <section className="d8-card mb-6 p-4">
        {!loaded ? (
          ...
          <>
            <div className="mb-4 flex flex-wrap items-center gap-2">
              <span
                className={`d8-chip font-bold ${
                  state.paper ? "bg-[#b6ff3c]/15 text-[#b6ff3c]" : "bg-[#ff6a4d]/20 text-[#ff6a4d]"
                }`}
              >
              ...
              <span
                className={`d8-chip font-semibold ${
                  engineAlive ? "bg-[#b6ff3c]/15 text-[#b6ff3c]" : "bg-[#ff6a4d]/15 text-[#ff6a4d]"
                }`}
              >
              ...
              <span
                className={`d8-chip font-semibold ${
                  state.running ? "bg-[#5a8cff]/15 text-[#5a8cff]" : "bg-[#ff6a4d]/15 text-[#ff6a4d]"
                }`}
              >
                ...
                <span
                  ...
                  className={`d8-chip ${
                    state.exchangesOk[ex]
                      ...
                      : "bg-[#ff6a4d]/10 text-[#ff6a4d]/80 line-through"
                  }`}
                >
                  {ex}
                </span>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
              {[
                ...
                <div key={label as string} className="d8-card px-3 py-2.5">
                  <p className="text-[10px] uppercase tracking-[0.14em] text-[var(--muted)]">
                    {label}
                  </p>
                  <p className={`d8-display text-2xl ${color}`}>{value}</p>
                </div>
              ))}
            </div>
            {state.lastError && (
              <p className="mt-3 rounded-lg bg-[#ff6a4d]/10 px-3 py-2 text-xs text-[#ff6a4d]">
                {state.lastError}
              </p>
            )}
          </>
      ...
      {/* controls */}
      <section className="d8-card mb-8 flex flex-wrap items-end gap-4 p-4">
        <div className="flex gap-2">
          <button
            onClick={() => control({ action: "start" })}
            disabled={busy || !engineAlive}
            className="d8-btn px-5 py-2 text-sm"
          >
            Start
          </button>
          <button
            ...
            disabled={busy || !engineAlive}
            className="d8-btn d8-btn-ghost px-5 py-2 text-sm"
          >
            Stop opens
          </button>
          <button
            ...
            className="d8-btn d8-btn-danger px-5 py-2 text-sm"
          >
            Kill switch
          </button>
        </div>
        <div className="flex flex-wrap items-end gap-3 text-xs">
          ...
          <button
            ...
            disabled={busy || !engineAlive}
            className="d8-btn d8-btn-ghost px-4 py-2"
          >
            Apply
          </button>
        </div>
      ...
      <section className="mb-8">
        ...
        {open.length === 0 ? (
          ...
          <div className="d8-card overflow-x-auto">
                  ...
                  <tr key={p.id} className="border-t border-[var(--border)] bg-[var(--surface)]/50">
                    ...
                    <td className="px-3 py-2 text-right">
                      <button
                        ...
                        disabled={busy || !engineAlive}
                        className="rounded-md border border-[var(--border)] px-2 py-1 text-xs hover:border-[#ff6a4d]/60 hover:text-[#ff6a4d] disabled:opacity-40"
                      >
                        Close
                      </button>
                    </td>
      ...
      <section className="mb-8">
        ...
        {opps.length === 0 ? (
          ...
          <div className="d8-card max-h-[28rem] overflow-auto">
            <table className="w-full text-sm">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                >
                  ...
                  <td className="px-3 py-2 text-right text-[#ff6a4d]">
                ...
                {byBase.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-right text-[#ff6a4d]">-{usd(r.feesUsd)}</td>
        ...
        <div className="mb-3 flex flex-wrap items-center gap-3">
          ...
          {closedShown.length > 0 && (
            <span className="text-xs text-[var(--muted)]">
              ...
              {" · "}fees <span className="text-[#ff6a4d]">-{usd(shownTotals.fees)}</span>
              ...
              <tbody>
                {closedShown.map((p) => (
                  <tr key={p.id} className="border-t border-[var(--border)] bg-[var(--surface)]/50">
                    ...
                    <td className="px-3 py-1.5 text-right text-[#ff6a4d]">
                ...
                >
                  ...
                  <span
                    className={`w-9 font-semibold uppercase ${
                      t.side === "buy" ? "text-[#b6ff3c]" : "text-[#ff6a4d]"
                    }`}
                  >
                    {t.side}
                  </span>
