// Funding-rate arbitrage engine — long-running process, started with:
//   npm run bot
// Paper mode by default (no API keys needed — public market data only).
// Live trading requires LIVE_TRADING=true plus API keys, and a restart.
// The dashboard (app/bot) talks to this process only through data/bot/ files.

import { hasCredentials, loadConfig, RUNTIME_TUNABLE } from "../lib/bot/config";
import { createExchanges, loadAllMarkets } from "../lib/bot/exchanges";
import {
  accrueFunding,
  Executor,
  LiveExecutor,
...
import { scan, snapshotIndex } from "../lib/bot/scanner";
import * as store from "../lib/bot/state";
import { decideCloses, decideOpen } from "../lib/bot/strategy";
import {
  BotConfig,
  ControlFile,
  EngineState,
...
async function main() {
  ...
  if (pairs.every((p) => !p.ok)) {
    console.error("No exchange reachable — check network/geo restrictions. Exiting.");
    process.exit(1);
  }
